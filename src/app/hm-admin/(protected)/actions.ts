...
export async function saveStoryAction(formData: FormData) {
  ...
  if (cover) {
    ...
    fileUpdates.cover_url = await uploadFile(supabase, cover, storyId, 'cover');
  }

  if (pdf) {
    validateFile(pdf, pdfTypes, pdfLimit, 'O PDF');
    fileUpdates.pdf_url = await uploadFile(supabase, pdf, storyId, 'pdf');
  }
  ...
  if (pageTexts.length > 0 || pageFiles.length > 0) {
    ...
    for (const file of pageFiles) {
      ...
      uploadedPages.push(await uploadFile(supabase, file, storyId, 'pages'));
    }
